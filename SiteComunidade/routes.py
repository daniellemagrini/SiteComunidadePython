...
@app.route("/")  # Caminho da home
def home():
    posts = Post.query.order_by(Post.id.desc())
    return render_template('home.html', posts=posts)

@app.route("/contato")  # Caminho do contato
def contato():
    return render_template('contato.html')

@app.route("/usuarios")  # Caminho do contato
@login_required
def usuarios():
    lista_usuarios = Usuario.query.all()
    return render_template('usuarios.html', lista_usuarios=lista_usuarios)

@app.route("/login", methods=['GET', 'POST'])  # Caminho do contato
def login():
    form_login = FormLogin()
    form_criar_conta = FormCriarConta()

    if form_login.validate_on_submit and 'bt_fazer_login' in request.form:
        ...
        if usuario and bcrypt.check_password_hash(usuario.senha, form_login.senha.data):
            ...
            flash('E-mail e/ou senha incorretos!', 'alert-danger')
    if form_criar_conta.validate_on_submit() and 'bt_criar_conta' in request.form:
        senha_criptografada = bcrypt.generate_password_hash(form_criar_conta.senha.data).decode("utf-8")
        usuario = Usuario(username=form_criar_conta.username.data, email=form_criar_conta.email.data, senha=senha_criptografada)
...
def sair():
    logout_user()
    flash('Logout realizado', 'alert-success')
    return redirect(url_for('home'))
